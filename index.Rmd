...
```
***
<iframe src="https://open.spotify.com/embed/playlist/37i9dQZF1EMfweBkW6hPXV" width="300" height="380" frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe>
...
```{r}
vinchen1 <-
  get_tidy_audio_analysis("7AnHS1o08tFXLmnOedcucv?si=2bba8cb44ac6432f") %>% # Change URI.
...
vinchen2 <-
  get_tidy_audio_analysis("1M4nbJUGkuBhI7Tx9lmeXp?si=e602daaa62524623") %>% # Change URI.
  compmus_align(bars, segments) %>%                     # Change `bars`
  select(bars) %>%                                      #   in all three
  unnest(bars) %>%                                      #   of these lines.
  mutate(
...
vinchen <-
  bind_rows(
    vinchen1 %>% mutate(category = "Un diavolo scaccia l'altro"),
    vinchen2 %>% mutate(category = "Retributor")
...
### Timbre Self-Similarity Matrix Un diavolo scaccia l'altro
```{r}
